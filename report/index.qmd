...
In 2024, UCLA cut its dedicated GIS librarian position. It's the only UC campus to have done so, and the data in this report show it's an outlier among national research peers as well.
...
p <- size_summary %>%
  ...
  geom_text(aes(label = paste0(n_with, "/", n)), vjust = 1.5, size = 3.5, color = "white") +
  ...
  labs(
    ...
    title = "Larger R1 institutions are far more likely to have dedicated GIS support"
  ) +
...
p2 <- sector_summary %>%
  ggplot(aes(x = sector, y = pct, fill = sector,
             text = paste0(sector, "<br>", n_with, " of ", n, "<br>",
                           percent(pct, accuracy = 1)))) +
  geom_col(width = 0.5, show.legend = FALSE) +
  geom_text(aes(label = paste0(n_with, "/", n)), vjust = 1.5, size = 4, color = "white") +
  scale_y_continuous(labels = percent_format(), limits = c(0, 1)) +
  scale_fill_manual(values = c("Public" = "#2c7bb6", "Private" = "#74c476")) +
  labs(
    x = NULL, y = "Share with dedicated GIS librarian",
    title = "Public R1s are somewhat more likely to have dedicated GIS support"
  ) +
  theme_minimal(base_size = 13)
...
p3 <- type_summary %>%
  ...
  geom_text(aes(label = paste0(n_with, "/", n)), hjust = 1.3, size = 3.5, color = "white") +
  scale_y_continuous(labels = percent_format(), limits = c(0, 1.1)) +
  coord_flip() +
  labs(
    x = NULL, y = "Share with dedicated GIS librarian",
    title = "Most large comprehensive R1s have dedicated GIS support"
  ) +
  theme_minimal(base_size = 13)
...
p4 <- team_summary %>%
  ggplot(aes(x = team_size_cat, y = n_insts,
             text = paste0(team_size_cat, " person(s)<br>",
                           ...
                           percent(pct, accuracy = 1), ")"))) +
  geom_col(fill = "#2c7bb6") +
  geom_text(aes(label = n_insts), vjust = 1.5, size = 4, color = "white") +
  labs(
    x = "GIS team size",
    y = "Number of institutions",
    title = "Many R1 institutions have built multi-person GIS teams"
  ) +
  theme_minimal(base_size = 13)
...
p5 <- tier_data %>%
  ...
  labs(
    ...
    y = "Share with dedicated GIS librarian",
    title = "Top R&D spenders almost universally have dedicated GIS librarians"
  ) +
  theme_minimal(base_size = 13)
...
UCLA is the only UC campus that had a GIS librarian and eliminated the role. UC Merced, which never had one, is now actively hiring. Every institution UCLA competes with for rankings, faculty, and research funding has kept the position; several have expanded it.
:::
...
UCSF is a health-sciences campus with no undergraduates, so its absence here isn't notable. Of the nine comprehensive UC campuses, six have dedicated GIS librarians. Merced has a position posted. Santa Cruz has nothing. UCLA has nothing, and unlike the others, it used to.
...
uc_data <- inst %>%
  ...
  mutate(
    ...
    status = if_else(has_gis,
                     paste0("✓ ", coalesce(as.character(n_staff), "0"), " staff"),
                     "✗ None"),
    staff_names = replace_na(staff_names, "—")
  ) %>%
...
Then the GIS librarian position was eliminated in 2024.

By December 2025, UCLA's ArcGIS Online environment was at 230–240% of its licensed storage capacity and growing at 5–10% per month. No one owned the platform. No retention policies existed. An internal OARC report prepared for the Executive Director of Research Computing put it plainly:
