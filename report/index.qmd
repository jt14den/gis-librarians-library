...
p <- size_summary %>%
  ...
  geom_text(aes(label = paste0(n_with, "/", n)), vjust = -0.4, size = 3.5) +
...
p2 <- sector_summary %>%
  ...
  geom_text(aes(label = paste0(n_with, "/", n)), vjust = -0.4, size = 4) +
  scale_y_continuous(labels = percent_format(), limits = c(0, 1)) +
...
p3 <- type_summary %>%
  ...
  geom_text(aes(label = paste0(n_with, "/", n)), hjust = -0.15, size = 3.5) +
...
p4 <- team_summary %>%
  ggplot(aes(x = team_size_cat, y = n_insts,
             text = paste0(team_size_cat, " person(s)<br>",
                           ...
                           percent(pct, accuracy = 1), ")"))) +
  geom_col(fill = "#2c7bb6") +
  geom_text(aes(label = n_insts), vjust = -0.4, size = 4) +
  labs(
    x = "GIS team size",
